...
fn main() {
    ...
    let possible_games = input
        ...
        .map(parse_game)
        .filter(|(_, draws)| {
            draws.into_iter().all(draw_is_possible)
        });

    let summed_ids = possible_games.map(|(index, _)| index)
        .sum::<i32>();

    println!("The sum of the ID's of the possible games is {summed_ids}");
}
...
fn parse_game(game_str: &str) -> (Id, Vec<Draw>) {
